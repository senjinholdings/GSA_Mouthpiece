...
class RankingApp {
    ...
    ensureComparisonStickyHeader(matrix) {
        ...
        if (!this.comparisonStickyHeader) {
            ...
            matrix.addEventListener('scroll', this.boundMatrixScrollHandler, { passive: true });
        }
    ...
    syncComparisonStickyHorizontalScroll() {
        ...
        inner.style.transform = `translateX(${-matrix.scrollLeft}px)`;
        ...
        if (itemCell) {
            itemCell.style.transform = `translateX(${matrix.scrollLeft}px)`;
        }
    }
