// where: public/app.js
// what: Main client logic for ranking page, including data loading and dynamic UI wiring.
// why: Ensure assets and JSON data resolve correctly even when the site is served from a subdirectory.
...
function resolveAssetPath(resource) {
    ...
    // Leave absolute/protocol-relative/data/mailto/tel untouched
    if (/^(?:[a-z][a-z0-9+.-]*:|\/\/|data:|mailto:|tel:)/i.test(resource)) {
        return resource;
    }
    // Remove leading './'
    let normalized = resource.startsWith('./') ? resource.slice(2) : resource;
    // Canonicalize any '../' or duplicate slashes using URL
    try {
        const u = new URL(normalized, 'https://asset-resolver.local');
        normalized = u.pathname;
    } catch (_) {
        // Fallback: strip leading '../' segments manually
...
function getDeploymentVariantId() {
    try {
        ...
        const match = base.match(/mouthpiece(\d{3})/);
...
class UrlParamHandler {
    ...
    getClinicUrlWithRegionId(clinicId, rank = 1, options = {}) {
        ...
        const opts = options || {};
        const ctaType = typeof opts.ctaType === 'string' && opts.ctaType.trim()
            ? opts.ctaType.trim()
            : null;
    ...
    getClinicUrlByNameWithRegionId(clinicName, options = {}) {
        ...
        const baseRedirect = this.getClinicUrlWithRegionId(clinicId, rank, options);
        if (!baseRedirect || baseRedirect === '#') {
            return '#';
        }
        ...
        if (utmCreative) {
            redirectUrl.searchParams.set('utm_creative', utmCreative);
        }
        if (gclid) {
    ...
    getDirectFormUrl(clinicId, rank = 1) {
        ...
        try {
            ...
            const getTrimmed = (fieldName) => {
                const raw = dm.getClinicText(clinicCode, fieldName, '');
            ...
            for (let i = 0; i < rankKeys.length; i++) {
                const candidate = getTrimmed(rankKeys[i]);
                if (candidate) {
                    return candidate;
                }
            }
            const commonKeys = ['直フォームURL', '直フォーム遷移先URL', '無料相談フォームURL', '予約フォームURL', 'フォームURL', '問い合わせフォームURL', 'CTA直リンクURL'];
            for (let i = 0; i < commonKeys.length; i++) {
                const candidate = getTrimmed(commonKeys[i]);
                if (candidate) {
                    return candidate;
                }
            }
...
class DisplayManager {
    ...
    updateRankingDisplay(clinics, ranking) {
        ...
        console.info('[app.js] updateRankingDisplay invoked', {
            clinicsCount: clinics ? clinics.length : 0,
            rankingRegion: ranking.regionId,
            ranks: ranking.ranks
        });
        ...
        sortedRanks.forEach(([position, clinicId]) => {
            ...
            console.debug('[app.js] rendering ranking card', {
                rank: rankNum,
                clinicId,
            ...
            const resolvedClinicLogoPath = typeof resolveAssetPath === 'function'
                ? resolveAssetPath(clinicLogoPath)
                : clinicLogoPath;

            const medalIconPath = typeof resolveAssetPath === 'function'
                ? resolveAssetPath(`/common_data/images/badges/rank-${rankNum}.svg`)
                : `/common_data/images/badges/rank-${rankNum}.svg`;

            rankingItem.innerHTML = `
                <div class="rank-medal ${medalClass}">
                    <img src="${medalIconPath}" alt="${medalText}" class="medal-image">
                </div>
                <div class="clinic-card">
                    ...
                    <div class="clinic-banner">
                        <img src="${resolvedClinicLogoPath}" alt="${clinic.name}バナー" onerror="this.style.display='none'">
                    </div>
...
class DataManager {
    constructor() {
        ...
        this.commonDataClinicMap = {}; // common_data配下のクリニック原データ（コード基準）
        this.rankingsByRegion = {}; // 地域ID→ランキング参照用マップ
        const ensureTrailingSlash = (value) => value.endsWith('/') ? value : `${value}/`;
        this.dataPath = ensureTrailingSlash(resolveAssetPath('/data/'));
        this.regionDataPath = ensureTrailingSlash(resolveAssetPath('/data/ranking/'));
        this.commonDataPath = ensureTrailingSlash(resolveAssetPath('/common_data/data/'));
    }

    logClinicTextsLoaded(sourceLabel) {
        if (typeof console === 'undefined' || typeof console.info !== 'function') {
            return;
        }
        const total = this.clinicTexts && typeof this.clinicTexts === 'object'
            ? Object.keys(this.clinicTexts).length
            : 0;
    ...
    async init() {
        try {
            ...
            // common_data配下の軽量JSONが存在する場合は上書きする
            await this.applyCommonDataOverrides();
    ...
    async loadFromBundle() {
        try {
            ...
            this.rankingsByRegion = this.createRankingsMap(this.rankings);
            ...
            this.logClinicTextsLoaded('bundle');
    ...
    async loadCommonTexts() {
        ...
        try {
            const override = await this.fetchJsonWithFallback([
                ...
                resolveAssetPath('/common_data/data/site-common-texts.json')
            ]);
        ...
        // ファビコンとヘッダーロゴを反映（ベースパス対応）
        if (this.commonTexts['ファビコン画像パス']) {
            const resolvedFavicon = typeof resolveAssetPath === 'function'
                ? resolveAssetPath(this.commonTexts['ファビコン画像パス'])
                : this.commonTexts['ファビコン画像パス'];
            const faviconElement = document.getElementById('favicon');
            if (faviconElement) {
                faviconElement.href = resolvedFavicon;
            }
            const headerLogoIcon = document.getElementById('header-logo-icon');
            if (headerLogoIcon) {
                headerLogoIcon.src = resolvedFavicon;
            }
        }
    }
    ...
    async fetchJsonWithFallback(candidates = []) {
        ...
        try {
            if (typeof RegionUtils?.getBaseRootPrefix === 'function') {
                const rootPrefix = RegionUtils.getBaseRootPrefix();
                if (rootPrefix) {
                    baseCandidates.push(rootPrefix.replace(/\/+$/, ''));
                }
            }
        } catch (_) {
            // ignore
        ...
        if (typeof __BASE_PATH_PREFIX === 'string' && __BASE_PATH_PREFIX) {
            baseCandidates.push(__BASE_PATH_PREFIX.replace(/\/+$/, ''));
        }

        if (typeof __BASE_PATH_RAW === 'string' && __BASE_PATH_RAW && __BASE_PATH_RAW !== '/') {
            baseCandidates.push(__BASE_PATH_RAW.replace(/\/+$/, ''));
        }
        ...
        const register = (url) => {
            ...
            const cleaned = url.replace(/\s+/g, match => encodeURIComponent(match));
            if (unique.has(cleaned)) return;
            unique.add(cleaned);
            queue.push(cleaned);
        };

        const addVariants = (originalUrl) => {
            if (!originalUrl) return;
            register(originalUrl);

            if (originalUrl.startsWith('/')) {
                const withoutLeading = originalUrl.replace(/^\/+/, '');
                register(`/${withoutLeading}`);
                register(`./${withoutLeading}`);
            }

            baseCandidates.forEach((basePrefixRaw) => {
                if (!basePrefixRaw) return;
                const basePrefix = basePrefixRaw.startsWith('/') ? basePrefixRaw : `/${basePrefixRaw}`;
                const regex = new RegExp(`^${escapeRegex(basePrefix)}(?=/|$)`);
                if (regex.test(originalUrl)) {
                    const stripped = originalUrl.replace(regex, '') || '/';
                    register(stripped);
                    if (stripped.startsWith('/')) {
                        register(`.${stripped}`);
                        register(`./${stripped.replace(/^\/+/, '')}`);
                    }
                }
            });
        };

        candidates.forEach(addVariants);
    ...
    async loadClinicTexts() {
        try {
            ...
            if (jsonData && typeof jsonData === 'object' && !Array.isArray(jsonData)) {
                ...
                this.logClinicTextsLoaded('clinic-texts.json');
                return;
            }
        } catch (err) {
        ...
        try {
            ...
            console.warn('⚠️ clinic-texts.csv の読み込みに失敗しました:', err);
            if (!this.clinicTexts || typeof this.clinicTexts !== 'object') {
                this.clinicTexts = {};
            }
        }
    }
    ...
    async loadClinicTextsFromCsvLegacy() {
        try {
            ...
            this.logClinicTextsLoaded('clinic-texts.csv');
        } catch (e) {
    ...
    async applyCommonDataOverrides() {
        const variant = getDeploymentVariantId();
        const fileName = variant ? `mouthpiece_clinics_data_${variant}.json` : 'mouthpiece_clinics_data_001.json';
        const candidates = [
            this.commonDataPath + fileName,
            resolveAssetPath(`/common_data/data/${fileName}`)
        ];

        let payload = null;
        try {
            payload = await this.fetchJsonWithFallback(candidates);
        } catch (err) {
            console.warn(`⚠️ ${fileName} の読み込みに失敗しました:`, err);
        }
        ...
        const rawClinics = Array.isArray(payload.clinics) ? payload.clinics : [];
        const normalizedClinics = this.normalizeCommonDataClinics(rawClinics);

        if (normalizedClinics.length > 0) {
            this.clinics = normalizedClinics;
        }

        const normalizedRankings = this.normalizeCommonRankings(payload.rankings, normalizedClinics);
        if (normalizedRankings.length > 0) {
            this.rankings = normalizedRankings;
            this.rankingsByRegion = this.createRankingsMap(normalizedRankings);
        } else {
            this.rankingsByRegion = this.createRankingsMap(this.rankings);
        }
        ...
        if (normalizedClinics.length > 0) {
            ...
            normalizedClinics.forEach((clinic) => {
                const raw = rawClinics.find((entry) => {
                    const entryId = entry && (entry.id ?? entry.code ?? entry.name);
                    return entryId && String(entryId) === clinic.id;
                }) || null;
                if (!raw) return;

                const textEntry = this.buildClinicTextEntryFromCommon(raw, clinic);
                overrides[clinic.name] = textEntry;
                const codeKey = clinic.code || clinic.id || clinic.name;
                if (codeKey) {
                    this.commonDataClinicMap[codeKey] = { raw, texts: textEntry };
                }
            });

            if (Object.keys(overrides).length > 0) {
                this.clinicTexts = { ...(this.clinicTexts || {}), ...overrides };
                this.logClinicTextsLoaded(fileName);
            }
        }
    ...
    normalizeCommonDataClinics(source = []) {
        return source
            .map((clinic) => this.transformCommonClinic(clinic))
            .filter(Boolean);
    }

    normalizeCommonRankings(source = {}, clinics = []) {
        ...
        return Object.entries(source)
            .map(([regionId, ranks]) => {
                const rankEntries = Array.isArray(ranks) ? ranks : [];
                const rankMap = {};
                rankEntries.forEach((clinicId, index) => {
                    if (!clinicId && clinicId !== 0) return;
                    const normalizedId = String(clinicId);
                    if (knownClinicIds.size === 0 || knownClinicIds.has(normalizedId)) {
                        rankMap[`no${index + 1}`] = normalizedId;
                    }
                });
                return {
                    regionId: String(regionId).padStart(3, '0'),
                    ranks: rankMap
                };
            })
            .filter((entry) => entry && Object.keys(entry.ranks).length > 0);
    }

    buildClinicTextEntryFromCommon(rawClinic, normalizedClinic) {
        const fallbackName = normalizedClinic?.name || rawClinic?.name || rawClinic?.['クリニック名'] || '';
        const pickValue = (keys = [], fallback = '') => {
            const keyList = Array.isArray(keys) ? keys : [keys];
            for (const key of keyList) {
                if (!key) continue;
                const value = rawClinic[key];
        ...
        const pickAsset = (keys = [], fallback = '') => {
            const value = pickValue(keys, fallback);
        ...
        const officialUrl = pickValue(['公式サイトURL', '遷移先URL（1位）'], '');
        const defaultComparisonText = pickValue(['説明文'], '詳細はお問い合わせください');

        return {
            'クリニック名': fallbackName,
            'ブランド名': pickValue(['ブランド名'], fallbackName),
            'INFORMATIONキャンペーンテキスト': pickValue(['INFORMATIONキャンペーンテキスト', 'キャンペーン内容（赤タグ内）', 'キャンペーン', 'キャンペーンテキスト'], ''),
            'INFORMATIONサブテキスト': pickValue(['INFORMATIONサブテキスト', '実績テキスト（ボタン上）', 'サブテキスト'], ''),
            'INFORMATION確認事項': pickValue(['INFORMATION確認事項', '確認事項', '注意事項', '免責事項'], ''),
            'CTAボタンテキスト': pickValue(['CTAボタンテキスト', 'CTAテキスト（ボタン内）', 'CTAテキスト'], ''),
            'CTAテキスト（ボタン内）': pickValue(['CTAテキスト（ボタン内）', 'CTAテキスト', 'CTAボタンテキスト'], ''),
            '公式サイトURL': officialUrl,
            '遷移先URL（1位）': officialUrl,
            '遷移先URL（2位）': pickValue(['遷移先URL（2位）'], officialUrl),
            '遷移先URL（3位）': pickValue(['遷移先URL（3位）'], officialUrl),
            'クリニックロゴ画像パス': pickAsset(['クリニックロゴ画像パス', 'ロゴ画像', 'ロゴ'], ''),
            'ロゴ画像パス': pickAsset(['ロゴ画像パス', 'ロゴ画像', 'ロゴ'], ''),
            '詳細バナー画像パス': pickAsset(['詳細バナー画像パス', 'バナー画像'], ''),
            'ポイント1': pickValue(['ポイント1'], ''),
            'ポイント説明1': pickValue(['ポイント説明1'], ''),
            'ポイント2': pickValue(['ポイント2'], ''),
            'ポイント説明2': pickValue(['ポイント説明2'], ''),
            'ポイント3': pickValue(['ポイント3'], ''),
            'ポイント説明3': pickValue(['ポイント説明3'], ''),
            'POINT1タイトル': pickValue(['POINT1タイトル', 'ポイント1'], ''),
            'POINT1内容': pickValue(['POINT1内容', 'ポイント説明1'], ''),
            'POINT2タイトル': pickValue(['POINT2タイトル', 'ポイント2'], ''),
            'POINT2内容': pickValue(['POINT2内容', 'ポイント説明2'], ''),
            'POINT3タイトル': pickValue(['POINT3タイトル', 'ポイント3'], ''),
            'POINT3内容': pickValue(['POINT3内容', 'ポイント説明3'], ''),
            'ランキングプッシュメッセージ': pickValue(['ランキングプッシュメッセージ', '説明文'], ''),
            '説明文': pickValue(['説明文'], ''),
            'INFORMATIONサブテキスト（ボタン上）': pickValue(['INFORMATIONサブテキスト（ボタン上）', '実績テキスト（ボタン上）'], ''),
            '総合評価': pickValue(['総合評価'], '4.5'),
            '比較表の注意事項': pickValue(['比較表の注意事項', '注意事項'], ''),
            '詳細_特徴タグ': pickValue(['詳細_特徴タグ', '特徴タグ'], ''),
            'メタディスクリプション': pickValue(['メタディスクリプション'], ''),
            'キャンペーンヘッダー': pickValue(['キャンペーンヘッダー', 'INFORMATIONヘッダー'], ''),
            'CTAボタン背景色': pickValue(['CTAボタン背景色'], ''),
            'CTAボタンテキスト色': pickValue(['CTAボタンテキスト色'], ''),
            'INFORMATIONボタンテキスト': pickValue(['INFORMATIONボタンテキスト'], ''),
            'INFORMATIONボタンリンク': pickValue(['INFORMATIONボタンリンク'], officialUrl),
            'CTAボタンリンク': officialUrl || '#',
            'comparison1': pickValue(['comparison1', '総合評価'], '4.5'),
            'comparison2': pickValue(['comparison2', '料金', 'ポイント1'], defaultComparisonText),
            'comparison3': pickValue(['comparison3', '人気', 'ポイント説明1'], defaultComparisonText),
            'comparison4': pickValue(['comparison4', '矯正範囲', 'ポイント説明2'], defaultComparisonText),
            'comparison5': pickValue(['comparison5', '目安期間'], '要問い合わせ'),
            'comparison6': pickValue(['comparison6', '通院頻度'], '要問い合わせ'),
            'comparison7': pickValue(['comparison7', '実績/症例数'], '実績多数'),
            'comparison8': pickValue(['comparison8', 'ワイヤー矯正の紹介'], defaultComparisonText),
            'comparison9': pickValue(['comparison9', 'サポート'], defaultComparisonText),
            'banner': pickAsset(['バナー画像', '詳細バナー画像パス'], '')
        };
    }

    transformCommonClinic(rawClinic) {
        if (!rawClinic) {
            return null;
        }
        const idSource = rawClinic.id ?? rawClinic.code ?? rawClinic.name ?? rawClinic['クリニック名'];
        const name = rawClinic.name ?? rawClinic['クリニック名'] ?? '';
        if (!idSource || !name) {
            return null;
        }
        ...
        const code = String(rawClinic.code ?? id);
        const sanitized = { ...rawClinic };
        sanitized.id = id;
        sanitized.name = name;
        sanitized.code = code;

        const knownAssetCodes = new Set([
            'dio','eminal','omt','tcb','sbc','rize','luna','kireiline','shinagawa','seishin','ws','lula','lieto','zenyum','invisalign','at','dsc','kbc','urara','tbs'
        ]);
        ...
        assetFields.forEach((field) => {
            if (sanitized[field]) {
                sanitized[field] = resolveAssetPath(sanitized[field]);
            }
        });

        const defaultBannerForCode = typeof resolveAssetPath === 'function'
            ? resolveAssetPath(`/common_data/images/clinics/${code}/${code}_detail_bnr.webp`)
            : `/common_data/images/clinics/${code}/${code}_detail_bnr.webp`;
        const defaultLogoForCode = typeof resolveAssetPath === 'function'
            ? resolveAssetPath(`/common_data/images/clinics/${code}/${code}-logo.webp`)
            : `/common_data/images/clinics/${code}/${code}-logo.webp`;

        if (!sanitized['バナー画像'] && knownAssetCodes.has(code)) {
            sanitized['バナー画像'] = defaultBannerForCode;
        }
        if (!sanitized['ロゴ画像'] && knownAssetCodes.has(code)) {
            sanitized['ロゴ画像'] = defaultLogoForCode;
        }

        if (!knownAssetCodes.has(code)) {
            sanitized['バナー画像'] = sanitized['バナー画像'] || '/images/ranking_header_banner.webp';
            sanitized['ロゴ画像'] = sanitized['ロゴ画像'] || '/common_data/images/favicon.png';
        }

        if (!sanitized['バナー画像']) {
            sanitized['バナー画像'] = (typeof resolveAssetPath === 'function' ? resolveAssetPath(`/common_data/images/clinics/${code}/${code}_detail_bnr.webp`) : `/common_data/images/clinics/${code}/${code}_detail_bnr.webp`);
        }
        if (!sanitized['ロゴ画像']) {
            sanitized['ロゴ画像'] = (typeof resolveAssetPath === 'function' ? resolveAssetPath(`/common_data/images/clinics/${code}/${code}-logo.webp`) : `/common_data/images/clinics/${code}/${code}-logo.webp`);
        }

        const withDefault = (fields, fallback) => {
        ...
        const defaultComparisonText = withDefault(['説明文', 'ポイント説明1', 'ポイント説明2', 'ポイント説明3'], '詳細はお問い合わせください');

        sanitized.comparison1 = sanitized.comparison1 || sanitized['総合評価'] || '4.5';
        sanitized.comparison2 = sanitized.comparison2 || withDefault(['料金', 'ポイント1'], defaultComparisonText);
        sanitized.comparison3 = sanitized.comparison3 || withDefault(['人気', 'ポイント説明1'], defaultComparisonText);
        sanitized.comparison4 = sanitized.comparison4 || withDefault(['矯正範囲', 'ポイント説明2'], defaultComparisonText);
        sanitized.comparison5 = sanitized.comparison5 || withDefault(['目安期間'], '要問い合わせ');
        sanitized.comparison6 = sanitized.comparison6 || withDefault(['通院頻度'], '要問い合わせ');
        sanitized.comparison7 = sanitized.comparison7 || withDefault(['実績/症例数'], '実績多数');
        sanitized.comparison8 = sanitized.comparison8 || withDefault(['ワイヤー矯正の紹介'], defaultComparisonText);
        sanitized.comparison9 = sanitized.comparison9 || withDefault(['サポート'], defaultComparisonText);

        return sanitized;
    }

    createRankingsMap(entries = []) {
        const map = {};
        entries.forEach((entry) => {
            if (!entry || !entry.regionId) {
                return;
            }
            map[entry.regionId] = entry;
        });
        return map;
    }
    ...
    getStoreImage(clinicCode, storeNumber) {
        ...
        // デフォルトの画像パス生成（ベースパス解決）
        const paddedNumber = String(storeNumber).padStart(3, '0');
        const path = `/images/clinics/${clinicCode}/${clinicCode}_clinic/clinic_image_${paddedNumber}.webp`;
        return typeof resolveAssetPath === 'function' ? resolveAssetPath(path) : path;
    }
    ...
    generateStoresDisplay(clinicId, regionId, providedStores = null) {
        ...
        const officialRedirectUrl = urlHandler.getClinicUrlWithRegionId(clinicId, rank, { ctaType: 'official' });
        const officialRedirectPayload = `JSON.stringify({clinic_id: '${clinicId}', rank: '${rank}', region_id: '${regionId}', cta_type: 'official'})`;
        ...
        visibleStores.forEach((store, index) => {
            ...
            const redirectUrl = officialRedirectUrl;
            const onclickHandler = targetUrl ? 
                `onclick="localStorage.setItem('redirectParams', ${officialRedirectPayload}); setTimeout(() => { window.open('${redirectUrl}', '_blank'); }, 10); return false;"` : '';
    ...
    // クリニックロゴパスを取得（ベースパス対応）
    getClinicLogoPath(clinicCode) {
        // キレイラインの特別処理
        const logoFolder = clinicCode === 'kireiline' ? 'kireiline' : clinicCode;
        const raw = this.getClinicText(clinicCode, 'クリニックロゴ画像パス', `/common_data/images/clinics/${logoFolder}/${logoFolder}-logo.webp`);
        return typeof resolveAssetPath === 'function' ? resolveAssetPath(raw) : raw;
    }
    ...
    getClinicDetailData(clinicId) {
        ...
        const detailData = {
            ...
            banner: this.getClinicText(clinicCode, '詳細バナー画像パス', (() => {
                ...
                const p = `/common_data/images/clinics/${bannerFolder}/${bannerFolder}_detail_bnr.webp`;
                return typeof resolveAssetPath === 'function' ? resolveAssetPath(p) : p;
            })()),
            features: (() => {
    ...
    mapRegionId(regionId) {
        ...
        if (existsInRegions) {
            const ranking = this.rankingsByRegion[paddedRegionId]
                || this.rankingsByRegion[unpaddedRegionId]
                || this.rankings.find(r =>
    ...
    getRankingByRegionId(regionId) {
        ...
        const paddedRegionId = String(mappedRegionId).padStart(3, '0');
        const unpaddedRegionId = String(parseInt(mappedRegionId, 10));

        let ranking = this.rankingsByRegion[paddedRegionId]
            || this.rankingsByRegion[unpaddedRegionId]
            || this.rankings.find(r => r.regionId === paddedRegionId || r.regionId === unpaddedRegionId);

        if (!ranking) {
            ranking = this.rankingsByRegion['013'] || this.rankings.find(r => r.regionId === '013');
        }

        return ranking || null;
    }

    getRankingsByRegion(regionId) {
        const ranking = this.getRankingByRegionId(regionId);
        if (!ranking || !ranking.ranks) {
            return [];
        }
        return Object.entries(ranking.ranks)
            .sort((a, b) => parseInt(a[0].replace('no', ''), 10) - parseInt(b[0].replace('no', ''), 10))
...
class RankingApp {
            ...
            fieldNames.forEach(fieldName => {
                ...
                if (fieldName === 'クリニック名') {
                    ...
                    if (!logoPath) {
                        ...
                        const logoFolder = clinicCode;
                        logoPath = typeof resolveAssetPath === 'function'
                            ? resolveAssetPath(`/common_data/images/clinics/${logoFolder}/${logoFolder}-logo.webp`)
                    ...
                    // データ由来の相対パス (../common_data/...) もベースパス解決
                    if (logoPath) {
                        logoPath = (typeof resolveAssetPath === 'function') ? resolveAssetPath(logoPath) : logoPath;
                    }
                    
                    const redirectUrl = this.urlHandler.getClinicUrlWithRegionId(clinic.id, clinic.rank || rankNum, { ctaType: 'official' });
                    const redirectPayload = `JSON.stringify({clinic_id: '${clinicId}', rank: '${rankNum}', region_id: '${regionId}', cta_type: 'official'})`;
                    const clinicNameOnclick = `onclick="localStorage.setItem('redirectParams', ${redirectPayload}); setTimeout(() => { window.open('${redirectUrl}', '_blank'); }, 10); return false;"`;
    ...
    generateComparisonTable(clinics) {
        ...
        clinics.forEach((clinic, index) => {
            ...
            const resolvedLogoPath = typeof resolveAssetPath === 'function'
                ? resolveAssetPath(logoPath)
                : logoPath;
            ...
            tr.innerHTML = `
                <td class="ranking-table_td1">
                    <img src="${resolvedLogoPath}" alt="${clinic.name}" width="80">
                    <a href="#" ${clinicNameOnclick} class="clinic-link" style="cursor: pointer;">${clinic.name}</a>
                </td>
                <td class="" style="">
    ...
    updateFirstChoiceRecommendation(topClinic) {
        ...
        const bannerPath = csvBannerPath ||
                             (typeof resolveAssetPath === 'function' ? resolveAssetPath(`/common_data/images/clinics/${clinicCode}/${clinicCode}_detail_bnr.webp`) : `/common_data/images/clinics/${clinicCode}/${clinicCode}_detail_bnr.webp`);
            ...
            bannerImage.src = typeof resolveAssetPath === 'function' ? resolveAssetPath(bannerPath) : bannerPath;
        ...
        if (infoLogo) {
            ...
            const logoPath = window.dataManager.getClinicText(clinicCode, 'meta13', '') || window.dataManager.getClinicText(clinicCode, 'クリニックロゴ画像パス', '') || 
                            `/common_data/images/clinics/${logoFolder}/${logoFolder}-logo.webp`;
            infoLogo.src = typeof resolveAssetPath === 'function' ? resolveAssetPath(logoPath) : logoPath;
    ...
    updateClinicDetails(clinics, ranking, regionId) {
        ...
        sortedRanks.forEach(([position, clinicId]) => {
            ...
            if (!data.banner) {
                const clinicCode = this.dataManager.getClinicCodeById(clinicId);
                const bannerFolder = clinicCode === 'kireiline' ? 'kireiline' : clinicCode;
                data.banner = (typeof resolveAssetPath === 'function' ? resolveAssetPath(`/common_data/images/clinics/${bannerFolder}/${bannerFolder}_detail_bnr.webp`) : `/common_data/images/clinics/${bannerFolder}/${bannerFolder}_detail_bnr.webp`);
            }
            ...
            const rankIconPath = (typeof resolveAssetPath === 'function'
                ? resolveAssetPath(`/common_data/images/rank_icon/rank${rank}.webp`)
                : `/common_data/images/rank_icon/rank${rank}.webp`);
            ...
            detailItem.innerHTML = `
                ...
                ${(() => {
                    ...
                    if (csvBannerRaw) {
                        const resolvedCsv = (typeof resolveAssetPath === 'function') ? resolveAssetPath(csvBannerRaw) : csvBannerRaw;
                        candidates.push(resolvedCsv);
                    }
                    for (let i = 2; i <= 10; i++) {
                        const p = `/common_data/images/clinics/${bannerFolder}/${bannerFolder}_detail_bnr${i}.webp`;
                        candidates.push(typeof resolveAssetPath === 'function' ? resolveAssetPath(p) : p);
                    }
                ...
                <div class="reviews-section">
                    ...
                    <section id="review_tab_box">
                        ...
                        ${(() => {
                            ...
                            const reviewIcons = [1,2,3,4,5,6,7,8,9].map(n =>
                                (typeof resolveAssetPath === 'function'
                                    ? resolveAssetPath(`/common_data/images/review_icon/review_icon${n}.webp`)
                                    : `/common_data/images/review_icon/review_icon${n}.webp`)
                            );
                ...
                <div class="campaign-section">
                    <div class="campaign-container">
                        ${(() => {
                            ...
                            const logoFolder = clinicCode === 'kireiline' ? 'kireiline' : clinicCode;
                            const logoSrc = (typeof resolveAssetPath === 'function'
                                ? resolveAssetPath(`/common_data/images/clinics/${logoFolder}/${logoFolder}-logo.webp`)
                                : `/common_data/images/clinics/${logoFolder}/${logoFolder}-logo.webp`);
                            ...
                            <div class="campaign-content">
                                <div class="camp_header3">
                                    <div class="info_logo">
                                        <img src="${logoSrc}" alt="${logoAlt}" onerror="this.onerror=null; this.src='${typeof resolveAssetPath === 'function' ? resolveAssetPath(`/common_data/images/clinics/${logoFolder}/${logoFolder}-logo.jpg`) : `/common_data/images/clinics/${logoFolder}/${logoFolder}-logo.jpg` }';">
                                    </div>
                                    <div class="camp_txt">
                                        ${campaignDescription}
                                    </div>
    ...
    handleImageError(imgElement, clinicName, storeNumber) {
        ...
        if (nextExtIndex < extensions.length) {
            ...
            // 全て失敗した場合、ロゴ画像にフォールバック（ベースパス対応）
            const fallbackWebp = typeof resolveAssetPath === 'function'
                ? resolveAssetPath(`/common_data/images/clinics/${clinicName}/${clinicName}-logo.webp`)
                : `/common_data/images/clinics/${clinicName}/${clinicName}-logo.webp`;
            const fallbackJpg = typeof resolveAssetPath === 'function'
                ? resolveAssetPath(`/common_data/images/clinics/${clinicName}/${clinicName}-logo.jpg`)
                : `/common_data/images/clinics/${clinicName}/${clinicName}-logo.jpg`;
            imgElement.src = fallbackWebp;
            imgElement.onerror = () => { imgElement.src = fallbackJpg; };
        }
    }
...
function initializeDisclaimers() {
    ...
    // ランキングデータを取得（旧DataManager互換フォールバック付き）
    let ranking = null;
    if (typeof window.dataManager.getRankingByRegionId === 'function') {
        ranking = window.dataManager.getRankingByRegionId(regionId);
    } else if (typeof window.dataManager.getRankingsByRegion === 'function') {
        const ranksArray = window.dataManager.getRankingsByRegion(regionId) || [];
        if (Array.isArray(ranksArray) && ranksArray.length > 0) {
            const ranksMap = {};
            ranksArray.forEach((clinicId, idx) => {
                if (clinicId && clinicId !== '-' && clinicId !== '') {
                    ranksMap[`no${idx + 1}`] = clinicId;
                }
            });
            ranking = {
                regionId: String(regionId).padStart(3, '0'),
    ...
    if (!ranking || !ranking.ranks || Object.keys(ranking.ranks).length === 0) {
        if (mainContent) {
            mainContent.innerHTML = '';
        }
        if (rankingDisclaimers) {
            rankingDisclaimers.innerHTML = '';
        }
        return;
    }
...
function bootstrapRankingApp(triggerLabel = 'direct-call') {
    console.info('[app.js] bootstrapRankingApp', { trigger: triggerLabel, readyState: document.readyState });
    const app = new RankingApp();
    window.app = app; // グローバルアクセス用
    console.info('[app.js] RankingApp instance created');
    app.init();
    console.info('[app.js] RankingApp.init() called');
    ...
    setTimeout(() => {
        try { initializeBannerSliders(); } catch (err) { console.warn('[app.js] initializeBannerSliders failed', err); }
    }, 200);
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => bootstrapRankingApp('domcontentloaded'));
} else {
    bootstrapRankingApp('document-already-ready');
}
