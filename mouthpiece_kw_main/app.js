...
class UrlParamHandler {
    ...
    getRegionId() {
        return this.getParam('region_id') || '000'; // デフォルトは全国
    }
...
class DataManager {
    ...
    getClinicDetailData(clinicId) {
        ...
        const detailData = {
                title: this.getClinicText(clinicCode, '詳細タイトル', 'マウスピース矯正プラン'),
            ...
            features: (() => {
                const tagsText = this.getClinicText(clinicCode, '詳細_特徴タグ', '# マウスピース矯正<br># 通院負担が少ない<br># 透明で目立たない');
                // <br>で分割し、#と空白を削除
                return tagsText.split('<br>').map(tag => tag.replace(/^#\s*/, '').trim()).filter(tag => tag);
            })(),
                priceMain: this.getClinicText(clinicCode, '人気プラン', 'マウスピース矯正コース'),
            priceValue: (() => {
                // 料金フィールドから月々の金額を抽出
...
class RankingApp {
    ...
    applyRegionLabels(region, options = {}) {
        ...
        if (rankRegionElement) {
            const baseText = (this.dataManager && typeof this.dataManager.getCommonText === 'function')
                ? this.dataManager.getCommonText('ランキング地域名テキスト', 'で人気のマウスピース矯正はココ！')
                : 'で人気のマウスピース矯正はココ！';
            if (isNational) {
    ...
    async updatePageContent(regionId) {
        try {
            ...
            if (rankingBannerImages.length > 0) {
                const altText = this.dataManager.getCommonText('ランキングバナーAltテキスト', 'で人気のマウスピース矯正はココ！');
    ...
    updateAllTexts(regionId) {
        try {
            ...
            if (siteLogo) {
                const logoText = this.dataManager.getCommonText('サイト名', 'マウスピース矯正ナビ');
            ...
            if (rankingBanner) {
                const rankingAlt = this.dataManager.getCommonText('ランキングバナーalt', 'で人気のマウスピース矯正はここ！');
            ...
            if (comparisonTitle) {
                const baseTitleText = this.dataManager.getCommonText('比較表タイトル', 'で人気のマウスピース矯正');
            ...
            if (comparisonSubtitle) {
                const subtitleHtml = this.dataManager.getCommonText('比較表サブタイトル', 'クリニックを<span class="accent-text">徹底比較</span>');
            ...
            if (detailsBannerImg) {
                const detailsBannerAlt = this.dataManager.getCommonText('案件詳細バナーalt', '費用×通いやすさ×保証で選ぶマウスピース矯正BEST3');
            ...
            if (footerSiteName) {
                const footerText = this.dataManager.getCommonText('サイト名', 'マウスピース矯正ナビ');
            ...
            if (footerCopyright) {
                const siteName = this.dataManager.getCommonText('サイト名', 'マウスピース矯正ナビ');
            ...
            if (tabTexts.length >= 3) {
                tabTexts[0].textContent = this.dataManager.getCommonText('Tipsタブ1タイトル', '費用と期間');
                tabTexts[1].textContent = this.dataManager.getCommonText('Tipsタブ2タイトル', '向いている人');
                tabTexts[2].textContent = this.dataManager.getCommonText('Tipsタブ3タイトル', '失敗しない選び方');
            }
    ...
    generateComparisonTable(clinics) {
        ...
        const rowConfigs = [
            ...
            { key: 'comparison2', headerKey: '比較表ヘッダー2', defaultLabel: '症例実績' },
            { key: 'comparison3', headerKey: '比較表ヘッダー3', defaultLabel: '特徴・特典' },
            { key: 'comparison4', headerKey: '比較表ヘッダー4', defaultLabel: '代表的な料金プラン' },
            { key: 'comparison5', headerKey: '比較表ヘッダー5', defaultLabel: '装置タイプ' },
            { key: 'comparison6', headerKey: '比較表ヘッダー6', defaultLabel: '通院頻度' },
            { key: 'comparison7', headerKey: '比較表ヘッダー7', defaultLabel: '対応症例' },
            { key: 'comparison8', headerKey: '比較表ヘッダー8', defaultLabel: '追加費用・保証' },
            { key: 'comparison9', headerKey: '比較表ヘッダー9', defaultLabel: 'サポート体制' },
                                ...
                                <div class=\"case-info\" style=\"margin-top: 5px; padding: 0 5%; text-align: left; font-size: 12px; line-height: 1.6; width: 100%;\">
                                    <table class=\"case-table\" style=\"width: 100% !important; border-collapse: collapse !important; font-size: 8px !important; line-height: 1.6 !important; display: table !important; table-layout: fixed !important;\">
                                        <tbody>
                                            ...
                                            ${buildRow('', '掲載している症例は代表的な治療例です。治療期間や仕上がりには個人差があり、効果を保証するものではありません。')}
                                        </tbody>
                                    </table>
                                </div>
                            </div>
